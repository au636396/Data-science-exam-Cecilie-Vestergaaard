---
title: "2_try_simple_model"
output: html_document
date: "2023-05-16"
---

This markdown includes Model 2: Animal type, age, breed, coat colour. Trained on the regular data, the 3 outcome values, and the "even" dataset.
...
df <- bigdf[c('animal_type', 'breed', 'color', "age_upon_intake_(years)", 'time_in_shelter_weeks')]
...
train_data <- data.matrix(train[,-5])                  # independent variables for train, here the variables of the trees
train_label <- train[,5]                                # dependent variables for train, here the speises of tree
  
test_data <- data.matrix(test[,-5])                    # independent variables for test, here the variables of the trees
test_label <- test[,5]  
...
#xgb.save(model, 'models/model_2_weeks')
model <- xgb.load('models/model_2_weeks')
```
...
```{r}
#getting the needed coulms 
df2 <- bigdf[c('animal_type', 'breed', 'color', "age_upon_intake_(years)", 'trinary')]
...
train_data2 <- data.matrix(train2[,-5])                  # independent variables for train
train_label2 <- train2[,5]                               # dependent variables for train
  
test_data2 <- data.matrix(test2[,-5])                    # independent variables for test
test_label2 <- test2[,5]                                 # dependent variables for test
...
```{r}
# saving the model
#xgb.save(model2, 'models/model_2_3out')

#loading in the model
model2 <- xgb.load('models/model_2_3out')
```
...
df3 <- bigdf3[c('animal_type', 'breed', 'color', "age_upon_intake_(years)", 'time_in_shelter_weeks')]
...
train_data3 <- data.matrix(train3[,-5])                  # independent variables for train, here the variables of the trees
train_label3 <- train3[,5]                                # dependent variables for train, here the speises of tree
  
test_data3 <- data.matrix(test3[,-5])                    # independent variables for test, here the variables of the trees
test_label3 <- test3[,5]  
...
```{r}
# loading and saving the model 
# xgb.save(model3, 'models/model_2_even')
model3 <- xgb.load('models/model_2_even')
```
